...
ames_new_2008 <- ames_raw_2008 %>% select(SalePrice, `Gr Liv Area`, `Overall Qual` , `Year Built`)
ames_new_2008  <- na.omit(ames_new_2008 )
func <- as.formula(log(SalePrice) ~ `Gr Liv Area` +  `Overall Qual` + `Year Built`)
y = log(ames_new_2008$SalePrice)
x = model.matrix(func, ames_new_2008)[,-1]
...
newx <- model.matrix(func, ames_raw_2009)[,-1]
pred_2009<-exp(predict(ridge,newdata=ames_raw_2009,newx = newx))
calc_loss(pred_2009,log(ames_raw_2009$SalePrice))

lasso <- glmnet(x, y, alpha = 1, lambda = lambda)
newx <- model.matrix(func, ames_raw_2009)[,-1]
pred_2009<-exp(predict(lasso,newdata=ames_raw_2009,newx = newx))
calc_loss(pred_2009,log(ames_raw_2009$SalePrice))
...
The lasso regression model performs slightly better than the ridge regression model given the result.
...
~~~
Ridge regression might not completely eliminate less important variables, so their impact may still be noticeable.
Lasso regression tends to be more consistent in identifying important variables by setting less important ones to zero.
...
library(leaps)
subsets <- regsubsets(Y ~ poly(X, 10, raw = TRUE), data = data, nvmax = 10)
summary_subsets <- summary(subsets)

par(mfrow = c(1, 3))
plot(summary_subsets$cp, xlab = "Number of Predictors", ylab = "Cp", type = "l")
plot(summary_subsets$bic, xlab = "Number of Predictors", ylab = "BIC", type = "l")
plot(summary_subsets$adjr2, xlab = "Number of Predictors", ylab = "Adjusted R^2", type = "l")

best_cp_model <- which.min(summary_subsets$cp)
best_bic_model <- which.min(summary_subsets$bic)
best_adjr2_model <- which.max(summary_subsets$adjr2)

coefficients_cp <- coef(subsets, id = best_cp_model)
coefficients_bic <- coef(subsets, id = best_bic_model)
coefficients_adjr2 <- coef(subsets, id = best_adjr2_model)
...
X_matrix <- model.matrix(~ poly(X, 10, raw = TRUE), data = data)
Y_vector <- data$Y
...
lasso_model <- glmnet(X_matrix, Y_vector, alpha = 1, lambda = optimal_lambda)
...
data_new <- data.frame(X = X, Y_new = Y_new)

subsets_new <- regsubsets(Y_new ~ poly(X, 10, raw = TRUE), data = data_new, nvmax = 10)

summary_subsets_new <- summary(subsets_new)

par(mfrow = c(1, 3))
plot(summary_subsets_new$cp, xlab = "Number of Predictors", ylab = "Cp", type = "l")
plot(summary_subsets_new$bic, xlab = "Number of Predictors", ylab = "BIC", type = "l")
plot(summary_subsets_new$adjr2, xlab = "Number of Predictors", ylab = "Adjusted R^2", type = "l")

best_cp_model_new <- which.min(summary_subsets_new$cp)
best_bic_model_new <- which.min(summary_subsets_new$bic)
best_adjr2_model_new <- which.max(summary_subsets_new$adjr2)

coefficients_cp_new <- coef(subsets_new, id = best_cp_model_new)
coefficients_bic_new <- coef(subsets_new, id = best_bic_model_new)
coefficients_adjr2_new <- coef(subsets_new, id = best_adjr2_model_new)

print(coefficients_cp_new)
print(coefficients_bic_new)
print(coefficients_adjr2_new)

lasso_cv_new <- cv.glmnet(X_matrix, Y_new, alpha = 1)

plot(lasso_cv_new)

optimal_lambda_new <- lasso_cv_new$lambda.min
cat("Optimal lambda:", optimal_lambda_new, "\n")

lasso_model_new <- glmnet(X_matrix, Y_new, alpha = 1, lambda = optimal_lambda_new)

coef_lasso_new <- coef(lasso_model_new)

print(coef_lasso_new)
